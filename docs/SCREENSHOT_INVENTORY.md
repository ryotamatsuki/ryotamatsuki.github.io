...
監査日: 2026-08-23 (JST)  
監査対象: `origin/main` 取得時点 `62ad0de93a94cf401a3a3c83c6c4b688612bca31` から作成した `feature/real-project-screenshots`

## 監査方針

- `projects.js` を Single Source of Truth として全14件を確認した。
- `appUrl` があるプロジェクトは、HTTP到達性を確認し、Cloud Browserで画面描画と主要UIを目視確認した。
- Streamlitはコールドスタートを考慮して待機した。`ehime-tour-planner` は入力UIまで描画できたため、短時間のHTTPタイムアウトだけで停止扱いにはしていない。
...
| `ehime-kokubunsai-ai-poc` | 愛顔えひめの文化祭 AI案内 | poc / Streamlit | repository only。公開アプリURLなし | — | AI, 行政・防災 | chat | 公開URLがないため画像化しない。CSS fallback |
...
| `bus-timelapse` | Bus Timelapse | poc / GitHub | repository only。公開アプリURLなし | — | 交通, GIS | bus | 公開URLがないため画像化しない。CSS fallback |
...
Featuredは3件を維持し、`ehime-kokubunsai-ai-poc` から `furusatonozei` に変更した。理由は、AI PoCには現在公開アプリURLがなく実画面を確認できない一方、ふるさと納税ダッシュボードは公開済みで、金額分布・KPI・比較軸が一枚で伝わり、避難/交通系との視覚的な多様性も確保できるためである。
